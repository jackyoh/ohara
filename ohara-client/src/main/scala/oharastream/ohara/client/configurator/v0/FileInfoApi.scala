...
object FileInfoApi {
  ...
  implicit val FILE_INFO_JSON_FORMAT: JsonRefiner[FileInfo] = JsonRefinerBuilder[FileInfo]
    .format(new RootJsonFormat[FileInfo] {
      override def read(json: JsValue): FileInfo = {
        ...
        new FileInfo(
          ...
          tags = value(TAGS_KEY) match {
            ...
            case j: JsValue =>
              throw DeserializationException(
                s"$TAGS_KEY must be mapped to JsObject but actual is ${j.getClass.getName}"
              )
          }
        )
      }
