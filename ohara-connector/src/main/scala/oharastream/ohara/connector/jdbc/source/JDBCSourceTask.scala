...
class JDBCSourceTask extends RowSourceTask {
  ...
  private[this] def queryData(startTimestamp: Timestamp, stopTimestamp: Timestamp): Seq[RowSourceRecord] = {
    ...
    try {
      ...
      val beforeExecuteQueryTimestamp = CommonUtils.current()
      val resultSet                   = prepareStatement.executeQuery()
      val afterExecuteQueryTimestamp  = CommonUtils.current()
      val counter = counterBuilder()
        .name("JDBC Query Time")
        .unit("Seconds")
        .document("Get query time from the database")
        .build()
      counter.setAndGet((afterExecuteQueryTimestamp - beforeExecuteQueryTimestamp) / 1000)
      try {
