...
class JDBCSourceTask extends RowSourceTask {
  ...
  private[this] def waitIfNeed(records: Seq[RowSourceRecord]): java.util.List[RowSourceRecord] = {
    val timeToWait = lastPoll + config.freq.toMillis - CommonUtils.current()
    if (records.isEmpty && timeToWait > 0) {
      println("Data is empty. To start sleep......")
