...
trait BaseQueryMode {
  /**
    * For implement SQL query function
    * @param startTimestamp start timestamp
    * @param stopTimestamp stop timestamp
    * @return RowSourceRecords
    */
  ...
  /**
    ...
    */
  protected[source] def isCompleted(startTimestamp: Timestamp, stopTimestamp: Timestamp, key: String): Boolean

  protected[source] def row(schema: Seq[Column], columns: Seq[ColumnInfo[_]]): Row = {
    Row.of(
      schema
        .sortBy(_.order)
